 ...
 * - V1.2.51 10-MAY-2011: Add the tool tips keyword definition for the {user_email}
 *                        Fix language text to avoid "ini" file error.
 * - V1.2.55 22-JUN-2011: Prepare the possibility to use Symbolic Links on Windows platform using PHP 5.3 or higher
 *                        AND Window (Vista, Win 7, Server 2008 or higher).
 *                        For the moment, this is not yet active as the delete of a slave site also delete files
 *                        and folders on Windows platform. So a patch is required in Joomla itself.
 * - V1.2.56 06-JUL-2011: Add keyword {site_id-n} description in the tool tips.
 * - V1.2.57 09-JUL-2011: Add Joomla 1.6 / 1.7 compatibilty to avoid that Joomla convert the ActionList combo box with HTML Encoding.
 *                        This avoid to have empty string when the Combo contain text with Accentuated character using the ISO-8859-1 as text.
 *                        The html encoding is a new feature introduced in Joomla 1.6 that is applied to all HTML combo box.
 *                        So to be compatible with Joomla 1.5, it is required to disable this html encoding.
 * - V1.2.66 02-OCT-2011: Give possibility to customize the getSiteIdsList().
 * - V1.2.69 14-NOV-2011: Allow the getTemplatesList() to have different $instance_suffix.
 *                        So that the list can be use in modules and present several times in a page with different instance suffix.
 * - V1.2.70 13-DEC-2011: Verify that DB connection exists inside getTemplateAdminName() function.
 *                        This avoid a Fatal Error in case where the DB connection failed (DB does not exists)
 * - V1.2.76 10-FEB-2012: Remove a warning message when computing the list of owners and when the Sites is empty.
 * - V1.2.80 19-MAR-2012: Add several keywords in the tooltips.
 *                        Change the getTemplateList to allow using a filter group array instead of a string.
 * - V1.2.86 17-APR-2012: Add several keywords in the tooltips for the parsing of the {site_alias} left, right, ...
 */
...
class MultisitesHelper
{
	...
	function isSymbolicLinks()
	{
		...
		if (!isset( $instance )) {
   	   ...
   	   if ( JUtility::isWinOS()) {
		   ...
		   // If Symbolic Links function does not exists, 
         else if ( !function_exists( 'symlink')) {
            // this probably mean that we are on Windows Platform where it is not allowed
		      // or that Windows platform runing a PHP < 5.3
   	      $instance = false;
   	   }
   	   // On Unix or Windows with PHP >= 5.3
   	   else {
         		...
         		if ( JFolder::exists( $log_dir) || JFile::exists( $log_dir)) {
                  ...
                  if ( function_exists( 'symlink') && @symlink( $log_dir, $link)) {
                     ...
                     if ( JFolder::exists( $fullname) || JFile::exists( $fullname)) {
                        $instance = true;
                        if ( JUtility::isWinOS()) {
                           // When working on Windows, it seems that link on a directory should be removed with a rmdir
                           // Try using the Remove Directory
                        ...
                        // If the link exist, try removing the link with Joomla API to potentially use the FTP Layer
                        if ( JFolder::exists( $fullname) || JFile::exists( $fullname)) {
                           // Remove the link
	...
	*/
	function getSiteIdsList( &$sites, $filter_site_ids, $fieldname='filter_site_ids', $unselectedTitle='Template Site', $onchange=' onchange="getUserList(this.options[selectedIndex].value);"')
	{
	   $rows = array();
	   if ( isset( $sites)) {
   	   foreach( $sites as $site) {
	...
	*/
	...
	{
		// Convert the Filter group into an array
