 ...
 * - V1.2.48 18-FEB-2011: Add a cache when computing the "countTables" to improve the performance
 *                        of the refresh icon. This avoid recomputing the number of table of the master
 *                        for each site.
 * - V1.2.56 24-JUN-2011: Add reading of All the Global Configuration parameters to potential use them in specific layout.
 *                        ie, allow display the MetaDescr of a slave site.
 *                        Prepare JMS 1.3.0 reading of "User Defined" parameters for the front-end website creation.
 *                        Prepare JMS 1.3.0 usage of "Parent Site ID" that could be used when filtering the list of slave site.
 * - V1.2.66 01-OCT-2011: Avoid to abort processing in case of DB Error.
 *                        So that the count table can continue to work and the other infos are computed.
 *                        Also check that site_id id provided when calling the 'load()' function.
 *                        Also return the "sitename" for the :master_db: in the getInstance().
 * - V1.2.73 29-JAN-2012: Fix reading of user define parameters.
 *                        Add definition of the "Meta Description" and "Meta Keywords"
 */
...
{
   var $parentSiteId    = '';       /**< Optional: Parent Site Identifier. */
   ...
   var $toMetaDesc      = '';       /**< Optional: New Meta Description */
   var $toMetaKeys      = '';       /**< Optional: New Meta Keywords */
   ...
   function load( $sitename)
   {
      ...
      if ( file_exists( $filename)) 
      {
         ...
         // If User Defined parameters are present, decode all fields
      ...
      if ( file_exists( $config)) {
         ...
         if ($handle) {
            while (!feof($handle)) {
               ...
               $line = trim( $line);
               // If lines start with 'var' or 'public'
               if ( (strpos( $line, 'var') === false) // Joomla 1.5
                 && (strpos( $line, 'public') === false) // Joomla 1.6
                  )
               {}
               else {
                  // Remove var, public
