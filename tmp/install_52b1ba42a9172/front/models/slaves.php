 ...
 * - V1.2.56 25-JUN-2011: Fix the total number of sites available for the pagination.
 *                        Add the possibility to refresh the list of slave site cache.
 * - V1.2.69 14-NOV-2011: Allow model be called from a "module" or other component.
 *                        Replace JPATH_COMPONENT_ADMINISTRATOR by JPATH_ADMINISTRATOR.'/components/com_multisites'
 * - V1.2.73 02-FEB-2012: Add possibility to get All Site info to populate the combo box.
 */
...
require_once( JPath::clean( JPATH_ADMINISTRATOR.'/components/com_multisites/libraries/joomla/application/component/model2win.php'));

require_once( JPath::clean( JPATH_ADMINISTRATOR.'/components/com_multisites/classes/site.php'));
require_once( JPath::clean( JPATH_ADMINISTRATOR.'/components/com_multisites/models/manage.php'));
...
class MultisitesModelSlaves extends JModel2Win
{
    ...
    */
   function &getAllSites( $forceRefresh=false)
   {
      $manage = new MultisitesModelManage();
	   $rows = $manage->getAllSites();
	   return $rows;
   }
    ...
    * @return the list of slave sites created by the user_id.
    */
   function &getUserSlaveSites( $user_id = null, $forceRefresh=false)
   {
      $manage = new MultisitesModelManage();
      ...
      // If registered user
	   ...
	   $rows = $manage->getSites( $forceRefresh);
	   $this->_countAll = $manage->getCountAll();  // Read the total number of sites available
      return $rows;
   }
