 ...
 * - 02-JUN-2011 V1.0.15 : Add the new Joomla 1.6 manifest file name based on the component name.
 * - 27-JAN-2012 V1.0.16 : Update error message for the english.
 * - 06-FEB-2012 V1.0.17 : Add verification of the registration information and in case where OVH return a cookie,
 *                         retry to get a new Product ID.
 * - 15-JUL-2012 V1.0.18 : Ensure that the class is not already defined by another extension
 * - 29-JUL-2012 V1.0.19 : Add a cache in getLatestVersion
 * - 04-AUG-2012 V1.0.20 : Add joomla 3.0 alpha 1 compatibility
 * - 04-DEC-2012 V1.0.21 : Fix joomla 1.5 compatibility to avoid calling the joomla "legacy" classes
 */
...
// --- Model legacy API class ---
// If Joomla legay model
...
class Edwin2WinModelRegistration extends Reg2WinModel
{
   ...
   function getRegistrationInfo( $prev_product_id = null)
   {
      ...
      // If the product ID is NOT found or has an invalid value returned by OVH (start with HTTP or contain Set-Cookie)
	...
	function getExtensionVersion()
	{
      ...
      if ( function_exists('jimport')) {
         ...
         if ( !file_exists( $filename)) {
            ...
            $filename = $path .DS. 'extension.xml';   // Now deprecated (2-jun-2011)
            if ( !file_exists( $filename)) {
   ...
   function getLatestVersion()
   {
      ...
      // Add a cache to avoid ping the website all the time to collect the information.
      if ( !empty( $results)) {
         return $results;
      }
