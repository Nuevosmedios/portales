 ...
 * - V1.2.71 06-JAN-2012: Add Joomla 1.7 compatibility
 */
...
class MultisitesInstallerLayout extends JObject
{
	...
	function install()
	{
		...
		$name = JFilterInput::clean( $manifest->name->data(), 'cmd');
		$this->set('name', $name);
		...
		if ($this->parent->parseFiles( $manifest->files, -1) === false) {
			// Install failed, rollback changes
			$this->parent->abort();
			return false;
		}
		if ($this->parent->parseFiles( $manifest->images, -1) === false) {
			// Install failed, rollback changes
			$this->parent->abort();
			return false;
		}
		if ($this->parent->parseFiles( $manifest->css, -1) === false) {
			// Install failed, rollback changes
			$this->parent->abort();
			return false;
		}
		...
		// Parse optional tags
		if ( !empty( $manifest->media))                     { $this->parent->parseFiles( $manifest->media, $clientId); }
		if ( !empty( $manifest->languages))                 { $this->parent->parseLanguages( $manifest->languages); }
		if ( !empty( $manifest->administration->languages)) { $this->parent->parseLanguages( $manifest->administration->languages, 1); }

		// Get the template description
		$description = $manifest->description;
		if ( is_a($description, 'JSimpleXMLElement')
		  || is_a($description, 'JXMLElement')
		   )
		{
	 ...
	 * @since	1.5
	 */
	function uninstall( $name, $clientId=0)
	{
		...
		$manifest =& $this->parent->getManifest();
		if (!is_a($manifest, 'JSimpleXML') && !is_a($manifest, 'JXMLElement')) {
		...
		// Remove files
		if ( !empty( $manifest->media))                     { $this->parent->removeFiles( $manifest->media, $clientId); }
		if ( !empty( $manifest->languages))                 { $this->parent->removeFiles( $manifest->languages); }
		if ( !empty( $manifest->administration->languages)) { $this->parent->removeFiles( $manifest->administration->languages, 1); }
