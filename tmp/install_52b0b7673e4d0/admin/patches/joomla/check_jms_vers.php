 ...
 * - V1.2.65 23-SEP-2011: Add the possibility to use arguments when processing patches.
 */
...
function jms2win_checkJMSVers( $model, $file)
{
   ...
   $jms_vers         = MultisitesController::_getVersion();
	...
	// Parse the $patchesVersion to remove potential addition string present in the version number.
	// Extract the first version number in format n.n.n
   ...
   // If Patch version is >= 1.2.69 and JMS version < 1.2.65
   else if ( (version_compare( $patchesVersion, '1.2.69') >= 0)
          && (version_compare( $jms_vers, '1.2.65') < 0))
   {
      // Then require update JMS kernel to 1.2.65 or higher
	   $rc = '[NOK]';
