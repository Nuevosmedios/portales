 ...
 * - V1.2.50 09-MAR-2011: Add toolbar for the About us menu.
 *                        Also compute the copyright date period.
 * - V1.2.54 02-JUN-2011: Fix the display of the about us.
 * - V1.2.73 29-JAN-2012: Fix the display of the about us to add Joomla 2.5 and redirect link "jms2win.com" on a new tab.
 * - V1.2.85 07-APR-2012: Add the "toSiteName" in the ajaxGetTemplate().
 */
   ...
   //------------ about ---------------
	function about()
	{
	   ...
	   $latestVersion  = $model->getLatestVersion();
		...
		JToolBarHelper::title( JText::_( 'About us'), 'config.png' );
		$yyyy = date( 'Y');
?>
<h3>Multisites for Joomla! 1.5.x, 1.6.x, 1.7.x, 2.5.x</h3>
<p>Version <?php
$getLatestURL = '';
$version = $this->_getVersion();
...
if ( !empty( $latestVersion['patch_version'])) {
	$parsed_patchesVersion = $patchesVersion;
...
<?php
   ...
   function ajaxGetTemplate()
   {
		...
		$result = 'template'
		        ...
		        . '|' . (!empty($template->toSiteName)     ? $template->toSiteName : '')
		        ;
		jexit( $result);
   }
