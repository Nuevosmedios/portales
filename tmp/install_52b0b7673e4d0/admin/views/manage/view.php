 ...
 * - 02-FEB-2012 V1.2.73 : Change dropdown computation values to be computed based on All Site and not only on the current page.
 * - 13-FEB-2012 V1.2.76 : Fix the dropdown computation that is also required when creating a new slave site.
 * - 21-MAR-2012 V1.2.81 : Accept to enter a site ID with an email address. Autorise the at (@) character.
 * - 04-APR-2012 V1.2.84 : Autorise the "-" inside a Site ID.
 */
	...
	function display($tpl=null)
	{
      ...
      if ( version_compare( JVERSION, '2.5') >= 0) { 
         ...
         if (JFactory::getUser()->authorise('core.admin', 'com_multisites')) {
         	JToolBarHelper::preferences('com_multisites');
         }
      }
