 ...
 * - V1.0.5 20-JUN-2011: Also autorize to call this API directly from Joomla.
 * - V1.0.6 04-MAR-2012: Use the CURL Header Size value to strip off the header for the getLastData().
 *                       This avoid to have the header remaining when the \n\n character sequence is not present
 *                       to mark the end of the HTTP Header.
 * - V1.0.7 07-OCT-2012: Make all functions static to avoid PHP Strict message
 */
...
class HTTP2Win
{
   ...
   //------------ getLastHttpCode ---------------
   /**
    * @brief Return the http header size of the last request.
    */
   ...
   //------------ setLastHeaderSize ---------------
   /**
    * @brief set a new Header Size corresponding to the last request.
    */
   ...
   static function getLastData()
   {
      ...
      // If we have a Header Size provided by CURL
      $headerSize = HTTP2Win::getLastHeaderSize();
      if ( $headerSize > 0) {
         // remove it
      ...
      // If the heaser size is unknown
      // Try searching for the \n\n that mark the end of the Header Size.
