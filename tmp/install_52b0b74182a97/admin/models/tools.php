 ...
 * - V1.2.53 27-MAY-2011: Fix a MySQL syntax error in Joomla 1.6 present in "_getmodule".
 *                        Also reduce the scope of the component displayed to the "site" (non core or not protected) in Joomla 1.6
 *                        to avoid list the "protected" components.
 * - V1.2.64 04-SEP-2011: Revamps the way that the extensions are saved. Use now the extension option instead of their name (title)
 *                        to be compatible with joomla 1.6/1.7 and later translate all the option into their title.
 *                        This allow a better matching of the extension and have extension title in clear on joomla 1.6 and 1.7.
 * - V1.2.67 08-OCT-2011: Fix warning message when call the MultisitesDatabase::createViews() function that was missing the new "{site_id}" parameter.
...
//   	      $tablesInfos = & $this->dbtables->getTablesInfos( $option);
    ...
    */
	function &getExtensions( $site_id)
	{
	   ...
	   // Compute the component name (title) and sort it
	   ...
	   // Compute the module name (title) and sort it
	   ...
	   // Compute the plugins name (title) and sort it
