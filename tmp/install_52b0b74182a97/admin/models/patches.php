 ...
 * - V1.2.50 19-APR-2011: Add default Joomla 1.6 patch deployment to avoid using the Joomla 1.5 one when a patch is not present for the give Joomla version.
 * - V1.2.55 08-JUN-2011: Add Joomla 1.7 compatibility
 * - V1.2.65 23-SEP-2011: Add the possibility to add arguments to "checking" functions.
 * - V1.2.71 28-DEC-2011: Add Joomla 2.5 compatibility
 */
   ...
   function _restoreInstallation()
   {
      ...
      else
      {
         ...
         if ( version_compare( JVERSION, '2.5') >= 0) {
      		$archivename = JPath::clean( JPATH_MUTLISITES_COMPONENT.'/patches/installation_j25.zip');
         }
         else if ( version_compare( JVERSION, '1.7') >= 0) {
      		$archivename = JPath::clean( JPATH_MUTLISITES_COMPONENT.'/patches/installation_j17.zip');
         }
   ...
   function _deployPatches( $patchfile='patches_files.tar.gz')
   {
      // Try to find the file in the "patches/jxxx/" directory where xxx is the Joomla version number
		...
		if ( empty( $archivename)) {
		   // If Joomla 2.5, try find a file with suffix _j25
         if ( version_compare( JVERSION, '2.5') >= 0) {
		   ...
		   // If Joomla 1.7, try find a file with suffix _j17
         else if ( version_compare( JVERSION, '1.7') >= 0) {
   	      ...
   	      // Use default path
      		...
      		if ( !JFile::exists( $archivename)) {
			   ...
			   // If Joomla 1.6, try find a file with suffix _j16
         ...
         // If Joomla 1.6 default rule is not OK, retry with Joomla 1.5
   		if ( empty( $archivename)) {
   	      // Use default path
   ...
   function isPatchesInstalled()
	{
	   ...
	   foreach( $patchlist as $file => $value) {
	      if ( is_array( $value)) {
	         // Use the first element as function name
