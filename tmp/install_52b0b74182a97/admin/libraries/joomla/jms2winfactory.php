 ...
 * - V1.2.48 17-FEB-2011: Replace all assign on _ressource by a call to set New DB Connection to access the protected field in Joomla 1.6.
 *                        This also allow manage the different field name "_resource" (j15) and "_connection" (j16).
 * - V1.2.49 08-MAR-2011: Sometime Joomla 1.6 send a Site ID as an array instead of a string.
 *                        So we detect this case in the Multisites DB connection to retreive the appropriate site ID value.
 * - V1.2.55 20-JUN-2011: Add Joomla 1.7 compatibility.
 * - V1.2.59 24-JUL-2011: Fix and error message when a patch is missing for Joomla 1.7.
 * - V1.2.64 07-SEP-2011: Fix the DB cache processing to save the "host/user" info in aim to get a new DB connection in case
 *                        where the user changed.
 * - V1.2.66 01-OCT-2011: Add possibility to avoid abort in case of DB Error. So that it is possible to "refresh" all the slave
 *                        sites that have a valid DB connection and ignore the wrong DB connections.
 * - V1.2.85 07-APR-2012: Improve message reporting when the connection to a DB is not possible.
 * - V1.2.87 24-APR-2012: Fix the checking on the DB connection to avoid using the error number of a previous call.
 *                        Now use connected() instead of getErrorNum().
 * - V1.3.00 18-JUN-2011: Add test on Geo Localisation.
 *                        Make the "/multisites" directory customizable.
 *
...
//   			$instance->setErrorInfo( 2, JTEXT::_('Unable to load Database Driver:') .$driver);
   		...
   		// In case of Joomla 1.7, call the patch to bypass the protected constructor
   		if ( method_exists( 'JDatabase', 'getAdapter')) {
   		   $instance	= JDatabase::getAdapter( $adapter, $options);
   		}
         else if ( version_compare( JVERSION, '1.7') >= 0) {
   			JError::raiseError(500, 'JDatabase patch is not installed');
//   			$instance->setErrorInfo( 2, 'JDatabase patch is not installed');
   			return null;
         }
   		else {
    ...
    */
	function &_createDBO_BasedOnConfig( $conf, $tempConnection=false, $abortOnError=true)
	{
		...
		if ( !$db->connected()) {
			if ( $abortOnError) {
			   ...
			   foreach( $options as $key => $value) { 
			      // Don't display the password (for security reason)
			      if ( $key == 'password') {}
			      else {
...
ob_end_clean();
Debug2Win::debug( 'DB Connection error stack = '.$stack);
*/
    ...
    */
	function &getMasterDBO( $tempConnection=false, $abortOnError=true)
	{
		static $instance;
    ...
    */
	function &getMultiSitesDBO( $aSite_id = null, $tempConnection=false, $abortOnError=true)
	{
		static $this_db;
    ...
    */
	function &getSlaveDBO( $site_id, $tempConnection=false, $abortOnError=true)
	{
		static $instances;
		static $none = null;
...
*/
   ...
   //------------ isGeoLocalisation ---------------
   /**
    * @return TRUE when the GeoLocalisation is enabled
    */
	function isGeoLocalisation()
	{
