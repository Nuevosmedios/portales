 ...
 * - V1.2.51 13-MAY-2011: Add the email notification processing
 * - V1.2.61 07-AUG-2011: Add display of fatal error message instead of using redirection to avoid infinite loop
 *                        in case where the error return on the same page.
 * - V1.2.73 29-JAN-2012: Add Joomla 2.5 compatibility to read the "layout" as a string and not as an array.
 *                        Add processing of Meta Description and Meta Keywords
 *                        Modify the rendring of the "Owner ID" list to let the list from all Sites and no more from the current page.
 * - V1.2.81 21-MAR-2012: Add a new deploy message when the prefix and alias is not present.
 * - V1.2.86 15-APR-2012: Add the possibility to enter an email address into the {site_alias} field
 *                        Also read the parameters from the "module ID" when it is present. Otherwise use the "component" parameters.
 * - V1.2.87 21-APR-2012: Add the processing of the onBeforeSave plugin return code to allow abort the processing in case where the plugin
 *                        generate an error.
 * - V1.2.89 06-JUN-2012: Add reading of the "Sender email" and "Sender name" parameters present in the "advanced settings".
 *                        They are used to customize the email notification.
 *                        Fix the read of JMS Layout parameter after that it were change for compatibility with Joomla 2.5
 * - V1.3.00 25-JUN-2011: Add saving and encoding of User Defined parameters that could be used in specific layout.
 *                        Fix limitstart pagination.
 * - V1.3.0b4 16-AUG-2012: Fix the layout computation to process the layout with additional parameters
 * - V1.2.96 27-OCT-2012: Add possiblity to call the "parent" display without calling the current class "display" function.
 *                        Fix layout sub-parameters retreive values unders joomla 2.5.
 */
...
class MultisitesViewSlaves extends JView2Win
{
    ...
    */
	function displayError( $errorMsg, $errorType="error")
	{
		$this->setLayout( 'error');
		
		$document = & JFactory::getDocument();
		$document->setTitle(JText::_('SLAVE_ERROR_TITLE'));

		// Assign view variable with will be used by the template
		$this->assignRef('errorMsg',  JText::_( $errorMsg));
		$this->assignRef('errorType', $errorType);
   ...
   //------------ parent_display ---------------
   /**
    * @brief Provide access to the parent "display" without call the "display" function.
    *        Can be used buy class that extends this call.
    */
	function parent_display($tpl=null)
	{
		// Display the template
		parent::display($tpl);
	}
	...
	function display($tpl=null)
	{
   	...
   	$option     = JRequest::getCmd('option');
		...
		if ( $user->get('guest') && $login_required) {
		   return $this->displayError( 'YOU MUST LOGIN FIRST');
		}
...
//		$filters['limitstart']	= $mainframe->getUserStateFromRequest( $option.'.limitstart',              'limitstart',        0, 'int' );
   ...
   //------------ _getViewLists ---------------
	function &_getViewLists( &$filters, $facultative_status=false, $onChangeStatus=false)
	{
	   $lists = array();
		...
		// Filter combo
		$lists['owner_id']   = MultisitesHelperSlaves::getUsersList( $this->allSites, $filters['owner_id']);
   	...
   	$option     = JRequest::getCmd('option');
		...
		if($edit) {
			...
			if ( ($table->owner_id == $user->id)
   		   ...
   		   return $this->displayError( 'SLAVE_EDIT_ERR_NOACCESS');
			}
		}
		else {
   ...
   //------------ _onBeforeSave ---------------
	function _onBeforeSave( &$enteredvalues, $template)
	{
		...
		// If Billable Website
		$eshop_events = JRequest::getBool('eshop_events', $this->getParams()->get('eshop_events', 0)==1);
		if ( !$eshop_events) {
		   return null;
		}
		
		// Call Plugin MultiSites and Trigger onBeforeSave()
		$d = array();
      ...
      // If there is a plugin that return false
    ...
    */
	function &getParams()
	{
	   ...
	   if ( empty( $instance)) {
         ...
         // If there is a jmslayout with sub-parameters, duplicate the sub-parameter to allow accessing them via a $param->get()
         $jmslayout = $instance->get( 'jmslayout');
         if ( !empty( $jmslayout)) {
            if ( $jmslayout['subparams']) {
               $subparams = $jmslayout['subparams'];
               if ( !empty( $subparams)) {
                  foreach( $subparams as $key => $value) {
                     $instance->def( $key, $value);
                  }
               }
            }
         }
    ...
    */
	function saveSlave($tpl=null)
	{
	   ...
	   // Remove special characters like the getCmd with additional at (@) to autorise email.
		$str = (string) preg_replace('/[^A-Z0-9_\.\-@]/i', '', JRequest::getString('site_alias', ''));
		$enteredvalues['site_alias']     = ltrim( $str, '.');
		...
		// Read the email notification parameters
		$enteredvalues['notifyUser']     = $params->get('email_Notification', 0) == 1;
		$enteredvalues['senderEmail']    = $params->get('sender_email', null);
		$enteredvalues['senderName']     = $params->get('sender_name', null);
		$enteredvalues['emailToAddress'] = $params->get('email_to_address', null);
		$enteredvalues['emailSubject']   = $params->get('email_subject', null);
		$enteredvalues['emailBody']      = $params->get('email_body', null);

		// Read the User Defined parameters
		if ( isset( $_REQUEST[ 'userdefs']))   {
		   if ( is_array( $_REQUEST[ 'userdefs'])) {
      		$enteredvalues['userdefs']     = array();
		      foreach( $_REQUEST[ 'userdefs'] as $key => $value) {
		         // Encode using base 64 to avoid hackers exploit this field
		         $enteredvalues['userdefs'][$key] = base64_encode( $value);
		      }
		   }
		}
		...
		// Can return true, false, null (case when billable website is not enabled)
		if ( $this->_onBeforeSave( $enteredvalues, $template) === false) {
		   if ( !empty( $enteredvalues['error_msg'])) { $msg = $enteredvalues['error_msg']; }
		   else                                       { $msg = '';}
			$this->_onDeploy_Err( $enteredvalues);
   	   // save the "entered values" into the view to be able re-using it for the redirect_url in the controler.
	   ...
	   $model = $this->getModel();
	   $rc    = $model->deploySite( $enteredvalues, true);
	   // save the "entered values" into the view to be able re-using it for the redirect_url in the controler.
	   $this->enteredvalues = $enteredvalues;
		if ( !$rc) {
		   $msg = $model->getError();
   	...
   	if ( !empty( $enteredvalues['status']) && $enteredvalues['status'] == 'Confirmed') {
		...
		if ( $this->isnew) {
		   ...
		   else if ( !empty( $this->site_prefix)) { $msgid = 'SITE_DEPLOYED_WITHOUT_ALIAS'; }
		   else                                   { $msgid = JText::sprintf( 'SITE_DEPLOYED_WITHOUT_ALIAS', $this->id); }
		}
		else {
   		$msgid = 'SITE_UPDATED';
		}
      ...
      if ( version_compare( JVERSION, '1.6') >= 0) {
         JFactory::$application->enqueueMessage($msg, 'message');
      }
      else {
