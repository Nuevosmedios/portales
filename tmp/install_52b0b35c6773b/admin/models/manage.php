 ...
 * - V1.2.51 15-MAY-2011: - Give the possibility to disable the session.cookie_domain computation in case where the hosting provider
 *                          has problem of session sharing (case of OVH shared hosting).
 * - V1.2.56 23-JUN-2011: - On Windows platform, try using "remove directory" to delete a symbolic link on a folder.
 *                          On Windows platform, delete a link on a folder does not work when using "unlink" function.
 *                        - Also add a cache when reading the site definition in the case where it would be called several times.
 *                          Case of front-end layout that might want recall the list of slave site with other filtering information.
 *                        - Add filtering on the "parent site id" when getting the list of slave sites.
 * - V1.2.61 02-AUG-2011: - Create the parent alias directory when it does not exists.
 * - V1.2.62 19-AUG-2011: - Add filtering of sites on the "table prefix".
 * - V1.2.64 04-SEP-2011: - Add joomla 1.6, 1.7 compatibilty to allow retreiving all the files in a folder including the hidden files like ".htaccess".
 *                        - Add the possibility to create Site ID with more than 20 characters when the letter tree is enabled.
 *                          To avoid the "infinite loop" detected in Joomla JFolder:create().
 *                        - On Joomla 1.6 and 1.7, verify that the "defines.php" is created in the root of the slave site to avoid infinite loop
 *                          when creating a fresh slave site. (Ping/Pong between installation and startup).
 * - V1.2.65 18-SEP-2011: - Add possibility to create complex view with a WHERE clause.
 *                        - Add a call to a multisites plugin when deleting a slave site.
 * - V1.2.73 29-JAN-2012: - Add the read of the multisites.cfg.php file to get the config in case where it is called from another extension.
 *                        - Add saving of the "user defined" parameters when they are present.
 *                        - Add saving of "Meta Description" and "Meta Keywords" in the configuration file.
 *                        - Add API to get the list of all sites.
 * - V1.2.78 04-MAR-2012: - Sent part of the GetWebsiteID info as a POST to avoid problem of filtering with special .htaccess
 *                          that could return a forbidden access when the GET contain pattern that might be rejected to reduce risk of hacking.
 * - V1.2.79 14-MAR-2012: - Fix the "Master Index Creation" when a new website contain a maximum limit of websites creation.
 *                          Force to recompute the list of website getSites( true) to ensure that the created site is included in the list.
 *                          Otherwise, with the cache, the new website was not present and therefore not recognized from the front-end.
 * - V1.2.81 21-MAR-2012: - Add the "newAdminEmail" in the Site Info to allow using it as keyword when computing the SiteID value.
 * - V1.2.84 04-APR-2012: - Autorize the "-" inside a Site ID.
 * - V1.2.85 07-APR-2012: - Add the possibility to compute the "Site Name" automatically based on a value setup in the JMS Template
 * - V1.2.86 16-APR-2012: - Read the "to email address" from a template when it is present.
 *                        - Add the "error_code" into the "enteredvalues" in case where the Site ID already exists.
 * - V1.2.87 21-APR-2012: - Add cleanup of the toDBName and toPrefix to remove all non alphanumeric characters. Just accept the underscore.
 * - V1.2.89 01-JUN-2012: - Add the possibility to ignore the "From Name" computation.
 *                          By default the config.FromName value is equal to the site name (title).
 * - V1.2.94 10-AUG-2012: - Fix the computation of the "toPrefix" after that non alphanumeric characters are filtered.
 */
...
include_once( JPATH_MULTISITES_COMPONENT_ADMINISTRATOR .DS. 'multisites.cfg.php');
...
class MultisitesModelManage extends JModel
{
             ...
             if ($file != "." && $file != "..") {
               ...
               // If this is a subdirectory
                  ...
                  if ( Site::is_Site( '', $filename)) {
                     ...
                     // If there is a filtering on the Parent Site ID and the parent id is empty of different of the select parent id
                     ...
                     if ( !empty( $filters['groupName'])) {
            	      ...
            	      // If there is a filtering on the Parent Site ID
                     if ( !empty( $filters['parentSiteId']) && ( empty( $site->parentSiteId) || $site->parentSiteId != $filters['parentSiteId']) ) {
                        $selected = false;
                     }
   ...
   //------------  getAllSites ---------------
   /**
    * @brief Return a list of ALL Sites.
    */

   function &getAllSites( $cleanFilter=array(), $forceRefresh=false)
	{
	   /* Collect all Site name when it is not already in the cache */
   ...
   function &getSites( $forceRefresh=false)
	{
	   ...
	   if ( !is_null($filters)) {
	      ...
	      // If there is a filtering on DB table Prefix
   ...
   function getCurrentRecord()
	{
		if ($this->_site == null) {
   	   ...
   	   // Remove special characters like the getCmd with additional at (@) to autorise email.
   ...
   //------------ canDelete ---------------
	...
	function _deleteFolderLinks($path)
	{
		...
		foreach ($folders as $folder) {
		   ...
		   if ( is_link( $folder)) {
   			$file = $folder;
   			// If Windows plaftorm, try using "remove directory" to delete the link
   			if ( JUtility::isWinOS() && @rmdir( $file)) {
   				// Do nothing
   			}
   			else if (@unlink($file)) {
   				// Do nothing
	...
	function delete()
	{
      ...
      $this->_deleteWebsiteID();
		...
		if ( !empty( $deploy_dir) && $deploy_dir != $site_dir) {
		   // If the "deploy_dir" is a parent of the master website directory, do not delete the deploy_dir
...
//      Debug2Win::debug_start( ">> _deployLinks() - START");
	   ...
	   if ( isset( $site_links)) {
         ...
         if ( version_compare( JVERSION, '1.6') >= 0) {
            // If the "define.php" description is not present
            if ( empty( $site_links['defines.php'])) {
               // Add the Multisites "defines.php" in the root directory to avoid loop during the install
...
//Debug2Win::debug_stop( '<< duplicateDBandConfig() - STOP');
   ...
   function _getWebsiteID( $enteredvalues)
   {
      ...
      $result = HTTP2Win::request( $url.$urlparams, $vars, 'POST');
      if ( $result === false) {
   ...
   //------------ createDir ---------------
   /**
    * @brief Create a directory that can be complex path when using the letter tree.
    * When letter tree is enabled, this require to split the creation in block of 20 directories
    * to avoid the infinite loop present in joomla.
    */
	function createDir( $path = '', $mode = 0755)
	{
	   // If letter tree is enable
      if ( defined( 'MULTISITES_LETTER_TREE') && MULTISITES_LETTER_TREE) {
         // Try split the directory creation in block of 10 subdirectories to avoid reach the limit of 20.
   ...
   function deploySite( &$enteredvalues, $front_end = false)
	{
      ...
      $month_str = array( 0, 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec');
	   ...
	   // Cleanup the toDBName to remove all non alphanumeric characters. Just accept the underscore.
	   ...
	   // Cleanup the toPrefix to remove all non alphanumeric characters. Just accept the underscore.
      ...
      if ( $front_end && empty( $enteredvalues['toPrefix'])) {
	   ...
	   if ( empty( $enteredvalues['emailToAddress']) && !empty($template->emailToAddress) ) {
	      // Do not evaluate the expression. It will be evaluated during the notification to use the "new admin user" info
...
// Don't write the password in clear	   if ( !empty( $enteredvalues['newAdminPsw']))    { $newDBInfo['newAdminPsw']   = $enteredvalues['newAdminPsw']; }
      ...
      if ( !empty( $config_dirs['alias_link'])) {
         ...
         if ( !empty( $deploy_dir) && Jms2WinFolder::exists( $deploy_dir)) {
            // If the alias parent directory does not exists
            $dir = dirname( $config_dirs['alias_link']);
            if ( !JFolder::exists( $dir)) {
               // Then create the parent alias directory
               JFolder::create( $dir);
            }
