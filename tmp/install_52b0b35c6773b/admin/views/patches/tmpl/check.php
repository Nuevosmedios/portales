...
if ( !empty( $this->latestVersion['patch_version'])) {
	$parsed_patchesVersion = $this->patchesVersion;
	if ( preg_match('#([0-9]+.[0-9]*.[0-9]+).*#i',$this->patchesVersion,$matches)) {
	   $parsed_patchesVersion = $matches[1];
	}
   if ( version_compare( $parsed_patchesVersion, $this->latestVersion['patch_version']) < 0) {
