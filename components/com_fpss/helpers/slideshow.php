<?php
/**
 * @version		$Id: slideshow.php 3415 2013-07-23 15:23:43Z joomlaworks $
...
class FPSSHelperSlideshow
{
	...
	public static function prepareSlide($slide, $params)
	{
		...
		switch($type)
		{
			...
			case 'com_hikashop' :
				$query = 'SELECT product.*, category.category_name, category.category_id FROM #__hikashop_product AS product 
				...
				WHERE product.product_id = '.(int)$id. ' GROUP BY product.product_id';
				$db->setQuery($query, 0, 1);
				$row = $db->loadObject();
				if (!is_null($row))
				{
					...
					{
						$slide->title = $row->product_name;
						$slide->text = $row->product_description;
					}
					$slide->category = $row->category_name;
					$slide->referenceCategoryID = $row->category_id;
					$slide->referenceAlias = ($row->product_alias == '') ? $row->product_name : $row->product_alias;
				}
				break;
	...
	public static function getSlideLink($slide)
	{
		...
		switch($type)
		{
			...
			case 'com_redshop' :
				jimport('joomla.filesystem.file');
				if (JFile::exists(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_redshop'.DS.'helpers'.DS.'redshop.cfg.php'))
				{
			...
			case 'com_hikashop' :
				...
				$category_pathway = '&'.$pathway_sef_name.'='.JRequest::getInt($category_pathway, 0).'&'.$related_sef_name.'='.$slide->referenceID;
				if (method_exists($application, 'stringURLSafe'))
				{
					$slide->referenceAlias = $application->stringURLSafe(strip_tags($slide->referenceAlias));
				}
				else
				{
					$slide->referenceAlias = JFilterOutput::stringURLSafe(strip_tags($slide->referenceAlias));
				}
				$link = hikashop_completeLink('product&task=show&cid='.$slide->referenceID.'&name='.$slide->referenceAlias.$urlItemID.$category_pathway);
				break;
	...
	{
		return base64_decode("PGRpdiBzdHlsZT0iZGlzcGxheTpub25lOyI+RnJvbnRwYWdlIFNsaWRlc2hvdyB8IENvcHlyaWdodCAmY29weTsgMjAwNi0yMDEzIEpvb21sYVdvcmtzIEx0ZC48L2Rpdj4=");
	}
	...
	public static function loadHeadData(&$params, $context = 'component', $moduleID = 0)
	{
		...
		if (version_compare(JVERSION, '3.0.0', 'ge')){
			...
			if ($jQueryHandling && strpos($jQueryHandling, 'remote') == true)
			{
				...
				} elseif($jQueryHandling=='1.9remote'){
