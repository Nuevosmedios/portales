...
d=[];var p=g.length,s=null,k=g.length;i.each(g.split(""),function(l,n){if(n=="?"){k--;p=l}else if(v[n]){d.push(RegExp(v[n]));if(s==null)s=d.length-1}else d.push(null)});return this.each(function(){function l(a){for(;++a<=k&&!d[a];);return a}function n(a){var b=i(this).caret();a=a.keyCode;t=a<16||a>16&&a<32||a>32&&a<41;if(b.begin-b.end!=0&&(!t||a==8||a==46))x(b.begin,b.end);if(a==8||a==46||z&&a==127){for(b=b.begin+(a==46?0:-1);!d[b]&&--b>=0;);for(a=b;a<k;a++)if(d[a]){j[a]=f.placeholder;var c=l(a);
...
c+1))}return p?h:s}var e=i(this),j=i.map(g.split(""),function(a){if(a!="?")return v[a]?f.placeholder:a}),t=false,w=e.val();e.data("buffer",j).data("tests",d);e.attr("readonly")||e.one("unmask",function(){e.unbind(".mask").removeData("buffer").removeData("tests")}).bind("focus.mask",function(){w=e.val();var a=q();u();setTimeout(function(){a==g.length?e.caret(0,a):e.caret(a)},0)}).bind("blur.mask",function(){q();e.val()!=w&&e.change()}).bind("keydown.mask",n).bind("keypress.mask",A).bind(y,function(){setTimeout(function(){e.caret(q(true))},
0)});q()})}})})(jQuery);
