<?php
/**
 * @file       install.multisitesusersite.php
 * @version    1.1.00
 ...
 * @par History:
 * - V1.1.00 21-SEP-2011: Initial Version
 */
...
// If Joomla 1.5, implement the com_install that redirect to Joomla 1.6 install.script.php
	...
	// Save the "parent" this
	$GLOBALS['installer_parent'] = &$this;
   ...
   function com_install()
   {
      ...
      if ( class_exists( $classInstallerScript)) {
         $j16Installer = new $classInstallerScript();
      	if ( method_exists( $classInstallerScript, 'preflight'))    { $j16Installer->preflight( 'install', $GLOBALS['installer_parent']); }
      	if ( method_exists( $classInstallerScript, 'install'))      { $j16Installer->install ( $GLOBALS['installer_parent']); }
      	if ( method_exists( $classInstallerScript, 'postflight'))   { $j16Installer->postflight( 'install', $GLOBALS['installer_parent']); }
      }
   }
} // Joomla 1.5