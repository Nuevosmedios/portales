<?php
/**
 * @version		$Id: slide.php 3411 2013-07-23 15:05:35Z joomlaworks $
		...
		switch($type)
		{
			...
			case 'com_virtuemart' :
				...
				else
				{
					...
					WHERE xref.virtuemart_product_id={$id} AND media.file_is_forSale = 0 AND media.file_is_downloadable = 0 AND published = 1";
					$db->setQuery($query);
			...
			case 'com_hikashop' :
				$db = JFactory::getDBO();
				$query = "SELECT * FROM #__hikashop_product WHERE product_id={$id}";
				$db->setQuery($query);
				$row = $db->loadObject();
				$response->title = $row->product_name;
				$response->text = $row->product_description;
				...
				$model->setState('id', $row->id);
				$image = $model->getRowImage();
				if($image)
				{
