<?php
/**
 * @version		$Id: menus.php 3411 2013-07-23 15:05:35Z joomlaworks $
...
class FPSSModelMenus extends FPSSModel
{
	...
	function getData()
	{
		...
		if ($this->getState('search'))
		{
			...
			{
				$conditions[] = "LOWER(title) LIKE ".$db->quote('%'.$db->escape($this->getState('search'), true).'%');
			}
			else
			{
		...
		if ($this->getState('search'))
		{
			foreach ($rows as $row)
			{
				$row->treename = isset($row->name) ? $row->name : $row->title;
				$menuItems[] = $row;
			}
		}
		else
