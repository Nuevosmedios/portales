<?php
/**
 * @version		13.8.227 libraries/eshiol/j2xml/importer.php
 * 
 * @package		J2XML
		...
		if ($import_users)
		{
			...
			foreach($xml->xpath("user[not(username = '')]") as $record)
			{
				...
				if (!$user || ($import_users == 2))
				{
					...
					{
						JLog::add(new JLogEntry(JText::sprintf('LIB_J2XML_MSG_USER_NOT_IMPORTED', $data['name'])),JLOG::WARNING,'j2xml');
						JLog::add(new JLogEntry($table->getError()), LOG::WARNING, 'j2xml');
					}
					$table = null;
				}
				elseif ($user)
				...
				if (!$category || ($import_categories == 2))
				{
					...
					if ($table->save($data))
					{
						if (!$category && ($keep_id == 1) && ($id > 1))
						{
							...
							{
								JLog::add(new JLogEntry(JText::sprintf('LIB_J2XML_MSG_CATEGORY_ID_PRESENT', $table->title)), JLOG::NOTICE, 'j2xml');
							}
						}
					...
					{
						JLog::add(new JLogEntry(JText::sprintf('LIB_J2XML_MSG_CATEGORY_NOT_IMPORTED', $data['title'])), JLOG::WARNING, 'j2xml');
					}
					$table = null;
				}
				elseif ($category)
				{
					$categories_id[$data['extension'].'/'.$section_alias.'/'.$alias] = $category->id;
					$categories_title[$data['extension'].'/'.$section_alias.'/'.$alias] = $category->title;
				}
			}
		}
		...
		{				
			...
			if (!$content || $import_content == 2)			
			{
				...
				if (!in_array(false, $result, true))
				{
					...
					{
						JLog::add(new JLogEntry(JText::sprintf('LIB_J2XML_MSG_ARTICLE_NOT_IMPORTED', $data['title'].' (id='.$id.')')), JLOG::WARNING, 'j2xml');
						if ($data['catid'])
							JLog::add(new JLogEntry($table->getError()), JLOG::WARNING, 'j2xml');
						else
							JLog::add(new JLogEntry(JText::sprintf('LIB_J2XML_MSG_CATEGORY_NOT_FOUND', $catid)), JLOG::WARNING, 'j2xml');
					}
				}
			}
		}
...
//						. ' AND level = 1'
					...
					{
						JLog::add(new JLogEntry(JText::sprintf('LIB_J2XML_MSG_WEBLINK_NOT_IMPORTED', $data['title'])), JLOG::WARNING, 'j2xml');
						JLog::add(new JLogEntry(JText::sprintf('LIB_J2XML_MSG_CATEGORY_NOT_FOUND', $data['catid'])), JLOG::WARNING, 'j2xml');
						continue;
					}
				}
				// Trigger the onContentBeforeSave event.
		...
		if ($import_images)
		{
			...
			{ 
				$src = JPATH_SITE.DS.str_replace('/', DS, html_entity_decode($image['src'], ENT_QUOTES, 'UTF-8')); 
				$data = $image;
				if (!file_exists($src) || ($import_images == 2))
				{
					...
					if (!JFolder::exists($folder)) {
						...
						{
							JLog::add(new JLogEntry(JText::sprintf('LIB_J2XML_MSG_ERROR_CREATING_FOLDER', $folder)),JLOG::WARNING,'j2xml');
							break;
						}
					}
 					if (JFile::write($src, base64_decode($data)))
						...
						JLog::add(new JLogEntry(JText::sprintf('LIB_J2XML_MSG_IMAGE_NOT_IMPORTED', $image['src'])),JLOG::WARNING,'j2xml');
				}
			}
		} 
